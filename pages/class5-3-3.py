...
with col2:
    ...
    st.session_state.model = st.selectbox(
        "AI模型",["gpt-4o-mini","gpt-4o","gpt-4o-search-preview",],
    )

with col3:
    if st.button("🗑️"):
        st.session_state.history = [] #按鈕按下時，清除對話紀錄
...
promt=st.chat_input('請輸入想要對話的訊息') #顯示對話輸入框，等待使用者輸入訊息
if promt: # 如果使用者輸入了訊息
    st.session_state.history.append({"role":"user","content":promt})
     #將使用者的訊息加入對話紀錄

    response = openai.chat.completions.create(
        model=st.session_state.model, #使用選定的AI模型
        messages=[{"role":"assistant","content":st.session_state.system_message}]
